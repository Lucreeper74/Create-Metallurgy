# Create Metallurgy - v1.0.4

### Additions
- Added Config with a few entries already

### Bug Fixes
- Fixed division by 0 when using Goggle on Foundry #159
- Fixed Wolframite Ore incorrect block sound #163
- Fixed Entity ingredient rendering in JEI/EMI
- Fixed compatibility molten metal bucket capability

### Gameplay Changes
- Adjusted faucet fluid stream width
- Faucet can no longer float in the air

### Art Changes
- Lower the Faucet model by one pixel to be centered
- Faucet fluid texture is now flowing instead of still

### API Changes

### Compatibility/Dependencies
- Fixed compatibility recipes (Tungsten Wires and experience nugget casting)
- Added Experience block casting with C:Enchantment Industry #148
- Updated to NeoForge 21.1.228