...
public class FoundryScenes {
    public static void foundryBasin(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        scene.effects().emitParticles(util.vector().centerOf(drainPos),
                scene.effects().simpleParticleEmitter(FluidFX.getFluidParticle(gold), Vec3.ZERO), 3, 20);
        ...
        scene.idle(10);
        scene.overlay().showControls(drainTop, Pointing.RIGHT, 30).withItem(CMFluids.MOLTEN_GOLD.get().getFluidType().getBucket(gold));
        scene.idle(10);
    }
    ...
    public static void alloying(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        scene.idle(5);
        scene.overlay().showControls(basinSide, Pointing.RIGHT, 30).withItem(CMFluids.MOLTEN_IRON.get().getFluidType().getBucket(iron));
        scene.idle(40);
