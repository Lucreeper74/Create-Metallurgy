...
public class CastingScenes {
    public static void castingBlocks(SceneBuilder scene, SceneBuildingUtil util) {
        ...
        scene.overlay().showControls(tableTop, Pointing.DOWN, 30)
                .withItem(ingot);
        scene.overlay().showControls(basinTop, Pointing.DOWN, 30)
                .withItem(block);
        scene.idle(20);
        ...
        scene.overlay().showText(80)
                ...
                .placeNearTarget();
        scene.idle(90);
        scene.overlay().showControls(filter, Pointing.LEFT, 30).rightClick()
                .withItem(ingot);
        scene.idle(7);
        scene.world().modifyBlockEntityNBT(funnelTable, FunnelBlockEntity.class, nbt -> {
            nbt.put("Filter", ingot.serializeNBT());
        });
        scene.idle(5);
        ...
        //Extract Items
        scene.world().modifyBlockEntity(basinFunnelPos, FunnelBlockEntity.class, funnel -> funnel.flap(false));
        scene.world().createItemEntity(util.vector().centerOf(basinFunnelPos), util.vector().of(0, 0, 0), block);
        scene.world().modifyBlockEntity(basinPos, CastingBasinBlockEntity.class, be -> be.inv
                .clearContent());
        scene.world().modifyBlockEntity(tableFunnelPos, FunnelBlockEntity.class, funnel -> funnel.flap(false));
